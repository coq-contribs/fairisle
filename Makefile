...
.PHONY: all opt byte archclean clean install depend html Libraries Fairisle
...
.depend depend:
	(cd Libraries ; $(MAKE) depend)
	(cd Fairisle ; $(MAKE) depend)

install:
	mkdir -p `$(COQC) -where`/user-contrib
	(cd Libraries ; $(MAKE) install)
	(cd Fairisle ; $(MAKE) install)

Makefile: Make
	mv -f Makefile Makefile.bak
	$(COQBIN)coq_makefile -f Make -o Makefile

	(cd Libraries ; $(MAKE) Makefile)
	(cd Fairisle ; $(MAKE) Makefile)

clean:
	rm -f *.cmo *.cmi *.cmx *.o *.vo *.vi *.g *~
	rm -f all.ps all-gal.ps $(HTMLFILES) $(GHTMLFILES)
	(cd Libraries ; $(MAKE) clean)
	(cd Fairisle ; $(MAKE) clean)

archclean:
	rm -f *.cmx *.o
	(cd Libraries ; $(MAKE) archclean)
	(cd Fairisle ; $(MAKE) archclean)

html:
	(cd Libraries ; $(MAKE) html)
	(cd Fairisle ; $(MAKE) html)
